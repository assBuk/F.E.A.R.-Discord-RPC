...
- Рефакторинг кода для удобочитаемости и поддержки: вынесены хелперы чтения памяти, улучшено прохождение цепочек указателей (поддержка 32/64-bit), добавлены регионы/комментарии и единый формат логирования ошибок.
- Подготовлена база для дальнейшей декомпозиции (разделение Program на MemoryReader/DiscordManager/SessionManager).
